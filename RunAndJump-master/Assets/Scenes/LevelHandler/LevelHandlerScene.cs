...
namespace RunAndJump {
	...
	public class LevelHandlerScene : BaseScene {
        ...
        public Lives LifeStorer;
        //;replace _lives with LifeStorer.
		#endregion
		...
		private const int TOTAL_LIVES = 1;
		...
		private void Start () {
            LifeStorer = GameObject.Find("LifeStorer").GetComponent<Lives>();
		...
		private IEnumerator InitSequence() {
            LifeStorer.LifeStorage = TOTAL_LIVES;
		...
		private IEnumerator InitScene() {
			...
			Transition.SetIntro (_levelId, _levelName, LifeStorer.LifeStorage);
		...
		private void UpdateGameInfoUI() {
			...
			GameInfo.SetLives(LifeStorer.LifeStorage);
		...
		private void RestartLevel() {
			...
			LifeStorer.LifeStorage--;
			if(LifeStorer.LifeStorage <= 0) {
